"""Ограниченный набор локальных настроек оператора.

Значения кладутся в файл на томе и читаются при каждом обращении. Список
ключей закрытый: интерфейс не может подменить произвольную переменную
окружения или адрес внешнего сервиса.
...
DEFAULT_FILE = "/var/lib/epgu-data/settings.env"
...
ALLOWED_KEYS = {
    ...
    # Реквизиты организации используются при формировании запросов на
    # сертификат и в других явных действиях оператора.
    "ORG_FULL_NAME",
    "ORG_SHORT_NAME",
    "ORG_INN",
    "ORG_OGRN",
...
# В публичной редакции сохраняемых секретов нет. Набор оставлен отдельным,
# чтобы правило было явным и не потерялось при добавлении настроек.
SECRET_KEYS = set()
...
def save(values: Mapping[str, str]) -> Dict[str, str]:
    ...
    body = [
        ...
        "# Локальный файл состояния; в git не попадает.",
        "",
    ]
...
def clear() -> Dict[str, int]:
    """Удалить все сохранённые локальные настройки."""
    path = settings_path()
    removed = len(load())
    try:
        path.unlink()
