...
| GET | `/mail/messages/{uid}/attachments/{index}/preview` | Что внутри вложения, без сохранения на диск |
| GET | `/mail/messages/{uid}/attachments/{index}/raw` | Само вложение байтами (`download=1` - сохранить себе) |
| GET | `/certsources/file` | Файл из каталога вложений (`path`, `download`) |
| GET | `/mail/messages/{uid}` | Одно письмо целиком: текст, вложения, статус запроса |
| GET | `/mail/messages/{uid}/reply` | Заготовка ответа: кому, тема, цитата, чем приложиться |
| POST | `/mail/reply` | Ответить в переписку (`uid`, `body`, `quote`, `attach`) |
| GET | `/mail/auto` | Состояние автоматической обработки почты и её журнал |
| POST | `/mail/auto` | Разрешить или запретить автоматику |
| POST | `/mail/auto/run` | Один такт обработки почты прямо сейчас |
| GET | `/mail/deadlines` | Запросы, ждущие подтверждения, и сколько осталось |
