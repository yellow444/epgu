...
.venv/Scripts/python -m uvicorn app:app --host 127.0.0.1 --port 55000
```

На Linux/macOS используйте `.venv/bin/python`. Не записывайте реальные
`apikey`, `KeyPin`, сертификаты или ключевые контейнеры в Git. Значения host-ов,
CORS и `SERVICES` задаются окружением; существующий профиль в `SERVICES`
объединяется с базовым глубоко и затем целиком валидируется.

## CryptoPro и публичная установка

Публичная установка работает как чистое Python-ядро без проприетарных пакетов
КриптоПро. Она публикует реестр и выполняет локальную подготовку, однако:

- `/status`, `/hc` и получение маркера доступа, которому нужна ГОСТ-подпись API-ключа, вернут `503` без `pycades`/CSP;
- реальная подпись требует отдельно лицензированного host-runtime с CryptoPro CSP, `pycades`, сертификатом и закрытым ключом;
...
Проприетарные дистрибутивы и личные ключи не должны попадать в wheel, sdist
или Git.
...
Полный прогон после подключения endpoint-ов Госключа: **44 passed, 5 skipped**;
пять пропусков зависят от отсутствующего лицензированного CSP/`pycades`. Общая
библиотека `epgu-api` проверяется отдельно полным suite; актуальный результат
формирует CI, а зафиксированный срез и coverage приведены в аудите. Проверка
