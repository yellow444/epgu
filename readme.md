...
| [`api-gosuslugi-client/`](./api-gosuslugi-client/) | React-интерфейс; воспроизводимая локальная и Docker-сборка использует Node.js 24 и `npm ci` |
...
Оба порта штатного Compose привязаны к `127.0.0.1`. Это намеренное ограничение: выбранный сертификат и токен ЕСИА хранятся в глобальном состоянии процесса, поэтому текущая конфигурация рассчитана на одного доверенного оператора и не должна публиковаться в сеть как многопользовательский сервис.

Публичный backend-образ намеренно не содержит проприетарные дистрибутивы КриптоПро CSP и `pycades`. Без них реестр и неподписывающие функции доступны, а операции подписи и health-check CryptoPro возвращают `503`. Для реальной подписи нужен отдельно лицензированный runtime/образ с CSP, `pycades`, сертификатом и закрытым ключом.
...
В текущем индексе `.env` не отслеживается, а `.env.example` содержит только плейсхолдеры - сами плейсхолдеры не являются секретами. Блокер остаётся в истории Git: в прежних ревизиях присутствовали `.env` и архив дистрибутива КриптоПро. До публичного релиза нужно отозвать/сменить любые когда-либо использованные значения, удалить чувствительные данные и проприетарный архив из всей истории, затем просканировать чистый клон. Подробный список блокеров - в [`docs/AUDIT_2026-08-12.md`](./docs/AUDIT_2026-08-12.md).
