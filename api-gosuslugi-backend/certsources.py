...
USB-токен доступен приложению только тогда, когда его видит установленный на
этом же хосте CryptoPro CSP. Модуль показывает фактические ридеры и ключевые
контейнеры провайдера и не пытается угадывать состояние устройства.
"""
...
def delete_certificate(
    ...
    *,
    ...
    Хранилищ на самом деле два, и это легко упустить. Обычное хранилище CSP
    и сертификат внутри ключевого контейнера обслуживаются отдельно.
    ...
    """
    if not cryptopro_available():
        raise RuntimeError("В этой установке нет КриптоПро, удалять нечем")
    if store not in ALLOWED_STORES:
        raise ValueError("Недопустимое хранилище")
...
def trust_test_ca(timeout: int = 30) -> Dict[str, Any]:
    ...
    for store, url, title in TEST_CA_CERTS:
        ...
        if result.returncode == 0:
            ...
            continue
        # Доверенные корни машины устанавливает администратор. Сохраняем файл в
        # предсказуемом месте и возвращаем оператору явное действие.
            ...
            pending.append(
                {
                    ...
                    "subject": info.get("subject", ""),
                    "file": parked,
                    "why": "нужны права администратора: установите корень на хосте и перезапустите backend",
                }
            )
        else:
...
# Если аппаратный датчик недоступен, используется биологический: он читает нажатия
...
def _request_error(output: str) -> str:
    ...
    return "Собрать запрос не удалось, подробности в журнале backend"


def readers_status() -> Dict[str, Any]:
    """Что КриптоПро видит на хосте: ридеры и ключевые контейнеры."""
    if not cryptopro_available():
...
def usb_guide() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "text": (
                "Backend сможет подписывать ключом с токена, если установленный "
                "на том же хосте CryptoPro видит ридер и ключевой контейнер."
            ),
            "commands": [],
        },
    ]
